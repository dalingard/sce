...
rescaled <- multiBatchNorm(Blk,Pet,Yan,mes1,mes2,mes3) #normalise
...
dec <- combineVar(Blk.dec, Pet.dec, Yan.dec,mes1.dec,mes2.dec,mes3.dec)
hvgs <- dec$bio > 0 #highly variable genes

corrected <- rescaleBatches(Blk,Pet,Yan,mes1,mes2,mes3) #remove batch effect

#Update sce with normalised and batch-corrected assay
cells <- c(Blk$sample_accession,Pet$sample_accession,Yan$sample_accession,mes1$sample_accession,mes2$sample_accession,mes3$sample_accession)
sce <- sce[univ, cells]
...
set.seed(100)
sce <- runPCA(sce, exprs_values="batch_corrected", subset_row=hvgs, ncomponents=35)
sce <- runUMAP(sce, dimred = 'PCA', external_neighbors=TRUE)

# Clustering
g <- buildSNNGraph(sce, use.dimred = 'UMAP', k=27)
...
early_blastocyst_hesc_sce <- sce
saveRDS(early_blastocyst_hesc_sce, file = "early_blastocyst_hesc_sce.rds")
